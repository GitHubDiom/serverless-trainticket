...
def write_csv(LOG_FILE):
    faas_pod_names = shell(
        'kubectl get pods --no-headers -o custom-columns=":metadata.name" -n openfaas-fn'
    ).output()

    log_file = open(LOG_FILE, 'w')
    print("function_name,hash_input,hash_output,duration(ms)",
          file=log_file,
          flush=True)
    for pod_name in faas_pod_names:
        print(pod_name)
        all_logs = shell(f'kubectl logs -n openfaas-fn {pod_name}').output()
...
def analyze_function_log(LOG_FILE):
    ...
    num_total_func = data['function_name'].nunique()
    num_all_invokes = data['function_name'].count()
    ...
    num_comp_invokes = comp_func['function_name'].count()
    num_comp_func = comp_func['function_name'].nunique()

    # Duplicated(keep=False) counts all same values.
    duplicated_func_of_computational = comp_func[comp_func.duplicated(
        ['function_name', 'hash_input'], keep=False)]
